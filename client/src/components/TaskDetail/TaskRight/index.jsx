...
const TaskRight = () => {
  ...
  function onSubmitDataTitle(e, title) {
    e.preventDefault();
    dispatch(
      updateTask({
        data: dataTitle,
        ...
        content: `đã thay đổi tên nhiệm vụ thành ${title}`,
      })
    );
    setDataTitle({ title: "" });
    closeTitleModel();
  ...
  function onSubmitDataDec(e) {
    ...
    dispatch(
      updateTask({
        ...
        idProject: project.data._id,
        idTask: task.data._id,
        content: `đã thay đổi mô tả nhiệm vụ thành ${dataDec.dec}`,
      })
    );
    setDataDec({ dec: "" });
    closeDecModel();
  ...
  const [colorChoice, setColorChoice] = useState(task.data.color);
  function onChangeInputDataTag(e) {
    const { name, value } = e.target;
    setDataTag({ ...dataTag, [name]: value });
  }
  function onSubmitDataTag(e) {
    e.preventDefault();
    dispatch(
      updateTask({
        data: { ...dataTag, color: colorChoice },
        token: auth.token,
        idProject: project.data._id,
        idTask: task.data._id,
        content: `đã thay đổi nhãn nhiệm vụ thành ${dataTag.tag}`,
      })
    );
    setDataTag({ tag: "", color: "" });
    closeTagModel();
  }
  ...
  function onChangeDeadline(e) {
    ...
    dispatch(
      updateTask({
        ...
        idProject: project.data._id,
        idTask: task.data._id,
        content: `đã thay đổi thời gian hoàn thành nhiệm vụ thành ${e}`,
      })
    );
  }
